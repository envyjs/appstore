...
let DEFAULT_HOME_DIRECTORY = ROOT + "/winefiles/";
let DEFAULT_BPP = 32;
let DEFAULT_ROOT_ZIP_FILE = "boxedwine.zip";
//params
let Config = {};
Config.urlParams = "/desktop=shell";
...
function getZlibParameter() {
  ...
  console.log(
    "setting use of internal boxedwine zip filesystem to: " + zlibEnabled
  );
  return zlibEnabled;
}
  ...
  buildExtraFileSystems(Buffer, function (extraFSs) {
    buildAppFileSystems(function (homeAdapter) {
      ...
      BrowserFS.FileSystem.XmlHttpRequest.Create(
        { index: rootListingObject, baseUrl: "" },
          ...
          function callback(e, contents) {
            ...
            BrowserFS.FileSystem.ZipFS.Create(
              ...
              function (e3, additionalZipfs) {
                ...
                homeAdapter = new BrowserFS.FileSystem.FolderAdapter(
                  "/",
                  additionalZipfs
                );
    ...
    function (e2, xmlHttpFs) {
      ...
      adapter.initialize(function dllcallback(de) {
        ...
        BrowserFS.FileSystem.OverlayFS.Create(
          { readable: adapter, writable: new BrowserFS.FileSystem.InMemory() },
          function (oe, overlay) {
            if (oe) {
              console.log(oe);
            }
...
function buildBrowserFileSystem(
  ...
  BrowserFS.FileSystem.OverlayFS.Create(
    { readable: mainfs, writable: new BrowserFS.FileSystem.InMemory() },
...
function postBuildFileSystem(rootFS, homeFS, extraFSs, dllFS) {
  var dirPrefixWithoutEndSlash = Config.dirPrefix.substring(
    0,
    Config.dirPrefix.length - 1
  );
  var mfs = new BrowserFS.FileSystem.MountableFileSystem();
  if (Config.isZlibEnabled) {
...
function deleteFile(fs, pathAndFilename) {
  try {
    ...
    console.log(
      "Unable to delete:" + "/root" + pathAndFilename + " error:" + ef.message
    );
  }
}
...
function execute(filename) {
  ...
  Config.WorkingDir =
    Config.dirPrefix.substring(ROOT.length) + path.substring(1);
  Config.Program = file;
...
function readFile(data, name, allFiles) {
  ...
  if (fileExt.toLowerCase() === "zip") {
    ...
    if (!createFolder(Config.dirPrefix, filenameNoExt)) {
